...
CrossCells_w_thresh(jk,:)=[Thresh length(FalseCells)];

CellSize_False(jk,:)=[mean(max(diameter1(:,FalseCells))') std(max(diameter1(:,FalseCells))')];
CellSize_all=[mean(max(diameter1)') std(max(diameter1)')];
   
CellShapeDiff_all=[mean(abs(diff(diameter1)')) std(abs(diff(diameter1)'))];

CellShapeDiff_False(jk,:)=[mean(abs(diff(diameter1(:,FalseCells))')) std(abs(diff(diameter1(:,FalseCells))'))];
...
%    figure(10);subplot(5,2,jk);
   ...
   txt1= [ 'Th=' num2str(Thresh) '']; 
   lx=randi(10);
   text(9,Cell_perc(9),txt1,'HorizontalAlignment','left','FontSize',18,'Color','k')
   end
...
FalseCells=find(falsecells>0);
    ...
    figure(101);subplot(3,3,8);bar(Ccount,'r');axis tight;box off
%     figure;hist(Ccount,length(CCC))
    ...
    for i=1:length(Xx)
       ...
       bigMatrix3=[(CrossCells_w_thresh(:,1)) [length(xy1)-CrossCells_w_thresh(:,2)] 100*CrossCells_w_thresh(:,2)/length(xy1) CellSize_False(:,1) CellShapeDiff_False(:,1)]
       
    
       
       T = array2table(bigMatrix3,'VariableNames',{'Thresh','TrueCells', 'FalseCellsRatio','Size','ShapeDiff'})
       
       
       writetable(T,'CrossTalk_output.csv');
       
       ss=num2str(bigMatrix3);
       
       [s,v] = listdlg('PromptString','Select a threshold now!',...
...
temp=ones(1,length(xy1));
temp(FalseCells)=0;

NewCellMatrix=[1:length(xy1); temp]';


TC=array2table(NewCellMatrix,'VariableNames',{'CellNo','Good_1_Bad_0'})

       writetable(TC,'NewCells.csv');
